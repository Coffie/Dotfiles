...
# Check brew cask installed formulaes that have auto_update==true
bau() {
  if __dotfiles_has_command brew && __dotfiles_has_command jq; then
    # shellcheck disable=SC2046
    brew info --cask --json=v2 $(brew ls --cask) | jq -r '.casks[]|select(.auto_updates==true)|.token'
  else
    echo "'brew' and 'jq' needed to run this command."
    return 1
  fi
}
